...
**`map_location="cpu"`**  
不加这个参数时，PyTorch 会把 checkpoint 中的 tensor 加载到它们**保存时所在的设备**（如 `cuda:0`）。在没有 GPU 的机器（CPU-only macOS / Windows）上加载 GPU 保存的 checkpoint 就会报错。`map_location="cpu"` 强制所有 tensor 先加载到 CPU，之后 `load_state_dict` 会自动把权重移到模型当前所在的设备（无论是 CPU、CUDA 还是 MPS）。
...
wandb api key：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
